...
Locator: locator('form').filter({ has: locator('form').filter({ hasText: 'Đăng nhậphoặc sử dụng tài kho' }) }).getByPlaceholder('Tài khoản')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('form').filter({ has: locator('form').filter({ hasText: 'Đăng nhậphoặc sử dụng tài kho' }) }).getByPlaceholder('Tài khoản')
