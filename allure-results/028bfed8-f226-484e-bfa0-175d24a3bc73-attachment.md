...
> 95  |     const parsed = courseInfoResponseSchema.parse(JSON.parse(responseText));
  ...
  114 |       expect(parsed.danhMucKhoaHoc.maDanhMucKhoaHoc).toBe(createdCourse.maDanhMucKhoaHoc);
  115 |     }
  116 | 
  117 |     logger.pass(`Get Course Info API passed for course: ${createdCourse.maKhoaHoc}`);
  118 |   });
  119 | });
  120 | 
```